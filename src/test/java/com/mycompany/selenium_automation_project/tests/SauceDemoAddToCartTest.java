...
public class SauceDemoAddToCartTest extends BaseTest {
	...
	public void TC_SD_005_removeItemFromCart_shouldEmptyCartAndHideBadge() {
	    ...
	    CartPage cart = products.openCart();
	    Assert.assertTrue(cart.isProductInCart(product), "Item should be in cart");
	    ...
	    By rowLocator = By.xpath("//div[@class='cart_item' and .//div[text()='" + product + "']]");
	    new WebDriverWait(driver, Duration.ofSeconds(10))
	            .until(ExpectedConditions.numberOfElementsToBe(rowLocator, 0));

	    Assert.assertFalse(cart.isProductInCart(product), "Item should be removed");
	}
}	
