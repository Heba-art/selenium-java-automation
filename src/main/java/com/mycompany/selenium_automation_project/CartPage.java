 ...
 // The cart description that contains the desired product name
    ...
    public void removeProductFromCart(String productName) {
        ...
        for (int attempt = 0; attempt < 2; attempt++) {
            ...
            // We wait for the button itself or the row to disappear.
            try {
                WebDriverWait shortWait = new WebDriverWait(driver, Duration.ofSeconds(6));
                shortWait.until(ExpectedConditions.or(
                        ExpectedConditions.stalenessOf(removeBtn),
        ...
        // After deletion, we verify that the row has indeed disappeared.
        wait.until(ExpectedConditions.numberOfElementsToBe(rowLocator, 0));

        // Finally, we check that the badge has disappeared from the basket icon.
