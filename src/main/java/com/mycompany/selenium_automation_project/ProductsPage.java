  ...
  //TC-SD-011
    ...
    public void addProductToCart(String productName) {
        ...
        By addBtn = By.cssSelector(
            "button#add-to-cart-" + slug + ", button[data-test='add-to-cart-" + slug + "']");
        By removeBtn = By.cssSelector(
            "button#remove-" + slug + ", button[data-test='remove-" + slug + "']");

        if (!driver.findElements(removeBtn).isEmpty()) return;

        WebElement add = new WebDriverWait(driver, Duration.ofSeconds(30))
        ...
        try {
            new WebDriverWait(driver, Duration.ofSeconds(5))
                    .until(ExpectedConditions.elementToBeClickable(add)).click();
        } catch (Exception e) {
            ((JavascriptExecutor) driver).executeScript("arguments[0].click();", add);
        }

     // Confirm transformation: Remove or badge = 1
        new WebDriverWait(driver, Duration.ofSeconds(10)).until(ExpectedConditions.or(
            ExpectedConditions.presenceOfElementLocated(removeBtn),
            d -> "1".equals(getCartBadgeText())
        ));
    }
